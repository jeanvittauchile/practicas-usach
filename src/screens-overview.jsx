  ...
  return (
    <div data-screen-label="Tabla de notas">
      <div className="section-head">
        ...
        <div className="actions">
          <button className="btn btn-secondary" onClick={() => setPrintOpen(true)}><I.print /> Imprimir</button>
          <button className="btn btn-secondary" onClick={() => setExportOpen(true)}><I.download /> Exportar PDF</button>
          <button className="btn btn-primary" onClick={() => setPublishOpen(true)}><I.send /> Publicar a estudiantes</button>
        </div>
      </div>
      ...
      {printOpen && <NotasPublishModal ctx={ctx} mode="print" onClose={() => setPrintOpen(false)} />}
      {publishOpen && <NotasPublishModal ctx={ctx} mode="publish" onClose={() => setPublishOpen(false)} />}
    </div>
  );
}

function NotasPublishModal({ ctx, mode, onClose }) {
  const meta = D.meta || {};
  const cols = D.NOTAS_COLUMNS || [];
  const todos = ctx.state.estudiantes || D.ESTUDIANTES;
  const [scope, setScope] = useState('todos');
  const estudiantes = scope === 'todos' ? todos : todos.filter(e => e.id === scope);
  const scopeLabel = scope === 'todos' ? 'Todos los estudiantes' : (todos.find(e => e.id === scope)?.nombre || '');
  const prof = window.readProfProfile ? window.readProfProfile() : {};
  const bodyRef = React.useRef(null);
  const title = 'Tabla de notas — ' + (meta.breadcrumb || 'Practica') + (scope !== 'todos' ? ' — ' + scopeLabel : '');
  ...
  function handlePublish() {
    ...
    openPrintWindow(bodyRef.current, title, true);
    ctx.toast && ctx.toast(scope === 'todos' ? 'Notas publicadas a todos los estudiantes' : `Notas publicadas a ${scopeLabel}`);
  }
    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        <div className="modal-head">
          <I.table />
          <h3 className="h3" style={{ margin: 0 }}>{mode === 'publish' ? 'Publicar tabla de notas a estudiantes' : 'Imprimir tabla de notas'}</h3>
          <button className="btn btn-ghost btn-sm" style={{ marginLeft: 'auto' }} onClick={onClose}><I.x /></button>
        </div>
        <div className="modal-body" style={{ paddingTop: 0 }}>
          <div style={{ display: 'flex', gap: 10, alignItems: 'center', padding: '16px 0 0' }}>
            <label className="muted" style={{ fontSize: 12.5, fontWeight: 600 }}>Alcance:</label>
            <select value={scope} onChange={e => setScope(e.target.value)}
                    style={{ padding: '7px 12px', border: '1.5px solid var(--border)', borderRadius: 8, fontSize: 13, fontFamily: 'inherit' }}>
              <option value="todos">Todos los estudiantes</option>
              {todos.map(est => <option key={est.id} value={est.id}>{est.nombre}</option>)}
            </select>
          </div>
          <div ref={bodyRef}>
        ...
        <div className="modal-foot">
          ...
          <button className="btn btn-primary" onClick={handlePublish}><I.send /> Publicar (PDF)</button>
        </div>
      </div>
    </div>
  );
